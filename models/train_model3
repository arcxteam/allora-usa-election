...
def train_model(token):
    ...
    # Define pipeline with StandardScaler and AdaBoost Regressor
    pipeline = Pipeline([
        ('scaler', StandardScaler()),  # Scaling the features
        ('adaboost', AdaBoostRegressor(base_estimator=DecisionTreeRegressor(max_depth=5), 
                                       n_estimators=100, learning_rate=1, random_state=42))  # AdaBoost Regressor
    ])
    ...
    save_path_model = os.path.join(model_file_path, f'adaboost_model_{token}.pkl')
